...
def test_create_lock_zero_balance(accounts, chain, token, voting_escrow, staking_contract, st_initial, st_extend):
    expected_unlock = chain.time() + st_initial
    voting_escrow.create_lock(10 ** 18, expected_unlock, {"from": accounts[0]})

    actual_unlock = voting_escrow.locked(accounts[0])[2]
